...
// Generate a synthetic forecast curve from actual data (since real model output isn't available client-side):
// produce predicted = smoothed actual + small bias + small uncertainty band.
function makePrediction(actualSeries) {
  // simple EMA of values to act as a model-like prediction
  const a = 0.18;
  let ema = actualSeries[0]?.f ?? 0;
  return actualSeries.map((p, i) => {
    ema = a * p.f + (1 - a) * ema;
    // gently bias the predicted curve so divergences show on charts
...
// =============== Sparkline ===============
function Sparkline({ data, w=120, h=22, color='currentColor', fill=true }) {
...
// =============== Generic line chart with optional second series ===============
function LineChart({ series, height=260, yLabel='L/min', xFormat, dual=false, bandSeries=null }) {
  const ref = useRef();
    ...
    <div ref={ref} style={{position:'relative'}}>
      ...
      >
        ...
        {ticks.map((t, i) => (
          <g key={i}>
            <line x1={pad.l} x2={pad.l+cw} y1={t.y} y2={t.y} stroke="var(--line)" strokeDasharray="2 4"/>
            <text x={pad.l-8} y={t.y+3} fontSize="10" fill="var(--ink-3)" fontFamily="var(--mono)" textAnchor="end">{t.v.toFixed(t.v<10?2:0)}</text>
          </g>
        ))}

        {/* Confidence band */}
        {bandSeries && (() => {
          const top = bandSeries.map((p,i) => `${i===0?'M':'L'}${x(i).toFixed(1)},${y(p.hi).toFixed(1)}`).join(' ');
          const bot = bandSeries.slice().reverse().map((p,i) => {
            const idx = bandSeries.length - 1 - i;
            return `L${x(idx).toFixed(1)},${y(p.lo).toFixed(1)}`;
          }).join(' ');
        ...
        {/* Series */}
        {series.map((s, si) => {
          const path = s.data.map((d,i) => `${i===0?'M':'L'}${x(i).toFixed(1)},${y(d.v).toFixed(1)}`).join(' ');
          const area = s.fill ? `${path} L${x(N-1).toFixed(1)},${y(0).toFixed(1)} L${x(0).toFixed(1)},${y(0).toFixed(1)} Z` : null;
          return (
            <g key={si}>
              {area && <path d={area} fill={`url(#g${si})`}/>}
              <path d={path} fill="none" stroke={s.color} strokeWidth={s.dashed ? 1.5 : 1.6} strokeDasharray={s.dashed ? '4 4' : ''}/>
            </g>
          );
        })}
        ...
        {/* x ticks */}
        {xTicks.map((i, k) => (
          <text key={k} x={x(i)} y={h-10} fontSize="10" fill="var(--ink-3)" fontFamily="var(--mono)" textAnchor="middle">
        ...
        {hover && (
          <g>
            <line x1={x(hover.idx)} x2={x(hover.idx)} y1={pad.t} y2={pad.t+ch} stroke="var(--line-2)"/>
            {series.map((s, si) => (
              <circle key={si} cx={x(hover.idx)} cy={y(s.data[hover.idx].v)} r="3.5" fill="var(--bg)" stroke={s.color} strokeWidth="1.6"/>
            ))}
          </g>
        )}
      </svg>
      ...
      {hover && (
        <div className="tip show" style={{ left: x(hover.idx), top: pad.t }}>
...
// =============== Bar chart ===============
function BarChart({ data, height=240, color='var(--accent)', label='liters' }) {
  const ref = useRef();
  const [size, setSize] = useState({w: 700, h: height});
  const [hover, setHover] = useState(null);
    ...
    <div ref={ref} style={{position:'relative'}}>
      <svg width={w} height={h}>
        {ticks.map((t,i)=>(
          <g key={i}>
            <line x1={pad.l} x2={pad.l+cw} y1={t.y} y2={t.y} stroke="var(--line)" strokeDasharray="2 4"/>
            <text x={pad.l-8} y={t.y+3} fontSize="10" fill="var(--ink-3)" fontFamily="var(--mono)" textAnchor="end">{fmt(t.v, 0)}</text>
          </g>
        ))}
        {data.map((d,i)=>{
          const bh = (d.v/max) * ch;
          ...
          return (
            <g key={i} onMouseEnter={()=>setHover(i)} onMouseLeave={()=>setHover(null)}>
              <rect x={x} y={y} width={Math.max(2, bw-4)} height={bh} fill={color} opacity={isHover ? 0.95 : 0.75}/>
              {i % Math.max(1, Math.floor(data.length/8)) === 0 && (
                <text x={x+(bw-4)/2} y={h-10} fontSize="10" fill="var(--ink-3)" fontFamily="var(--mono)" textAnchor="middle">{d.t}</text>
              )}
            </g>
          );
        })}
...
Object.assign(window, { fmt, pad, tsLabel, makePrediction, Sparkline, LiveStrip, LineChart, BarChart });
