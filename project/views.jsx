/* global React, fmt, Sparkline, LiveStrip, LineChart, BarChart, makePrediction */
const { useState, useEffect, useMemo, useRef } = React;

// =============== Topbar / Rail ===============
function Rail({ tab, setTab, lastReading, meter, setMeter, meters }) {
  const items = [
  { group: 'Monitor', items: [
...
function Topbar({ tab, clock }) {
  ...
  return (
    <div className="topbar">
      <h1><span className="muted">Smart Water Meter</span><span className="crumb">/</span>{titles[tab]}</h1>
      <div className="meta">
...
// =============== Overview ===============
function Overview({ data, scope, setScope, simulating, anomalies }) {
  const today = data.today;
  // current flow = last point
  ...
  // hourly liters for current scope
    ...
    <>
      <div className="hero">
        <div className="lead">
          ...
          <div className="delta">
            Today <span className={'pill ' + (delta > 0 ? 'up' : 'down')}>{delta > 0 ? '▲' : '▼'} {Math.abs(delta).toFixed(1)}%</span> vs yesterday
          </div>
          <div className="sub">
            Streaming from <span className="mono">WM-10293</span>. Forecast model is a BiLSTM-Attn × PatchTST ensemble served from the inference API; updates hourly with the latest 168h context window.
          </div>
        </div>
      ...
      <div className="kpis">
        ...
        <div className="kpi">
          ...
          <div className="spark"><Sparkline data={sparkFlow.map((v, i) => v + Math.sin(i / 8) * 0.05)} w={140} color="var(--accent-2)" /></div>
        </div>
        <div className="kpi">
          ...
          <div className="spark">
            <svg width="140" height="22">
              <line x1="0" y1="11" x2="140" y2="11" stroke={anomalies.length ? 'var(--bad)' : 'var(--good)'} strokeDasharray="2 4" />
            </svg>
          </div>
        </div>
      </div>
      ...
      <div className="section">
        <h2>
          ...
          <span className="aside">
            <Scope scope={scope} setScope={setScope} />
          </span>
        </h2>
        <div className="panel">
          <div className="head">
            ...
            <div className="legend">
              <span className="sw" style={{ color: 'var(--accent)' }}><i style={{ background: 'var(--accent)' }}></i>Actual flow</span>
            </div>
          </div>
          <LineChart
            height={260}
...
function Forecast({ data }) {
  ...
  const series = useMemo(() => {
    let actual;
    if (scope === '1d') actual = data.today.map((p) => ({ t: p.t.slice(11, 16), v: p.f }));else
    if (scope === '7d') actual = data.hourly.slice(-7 * 24).map((p) => ({ t: p.t.slice(5, 13), v: p.l / 60 }));else
    actual = data.daily.slice(-30).map((p) => ({ t: p.d.slice(5), v: p.l / 1440 }));
    const pred = makePrediction(actual.map((p) => ({ t: p.t, f: p.v }))).
    map((p) => ({ t: p.t, v: p.f }));
  ...
  // metrics
  const errs = series.actual.map((a, i) => Math.abs(a.v - series.pred[i].v));
  const mae = errs.reduce((s, x) => s + x, 0) / errs.length;
  const meanA = series.actual.reduce((s, p) => s + p.v, 0) / series.actual.length;
  const mape = errs.reduce((s, x, i) => s + (series.actual[i].v > 0 ? x / series.actual[i].v : 0), 0) / errs.length * 100;
      ...
      <div className="section tight">
        ...
        <div className="panel">
          <div className="head">
            ...
            <div className="legend">
              <span className="sw" style={{ color: 'var(--accent)' }}><i style={{ background: 'var(--accent)' }}></i>Actual</span>
...
// =============== Anomalies ===============
function Anomalies({ events, onSimulate, simulating }) {
  const sevColor = { bad: 'var(--bad)', warn: 'var(--warn)', info: 'var(--ink-2)', ok: 'var(--good)' };
  return (
    <>
      <div className="section">
        <h2>
          ...
          <span className="aside">
            <button className={"btn " + (simulating ? 'danger' : 'primary')} onClick={onSimulate}>
              {simulating ? '■  Stop leak simulation' : '■ Stop leak simulation'}
            </button>
          </span>
        </h2>
...
function Plan() {
  const weeks = [
  { w: 'Week 1', h: 'Firmware + ingest pipeline',
    b: 'ESP32 sketch with hall-sensor pulse counting and MQTT publish at 5-second cadence; Mosquitto broker; Node gateway subscribes, validates, batches into Mongo. Verify end-to-end with a single bench meter.' },
  { w: 'Week 2', h: 'Storage schema + REST API',
    b: 'Mongo time-series collection; hourly and daily materialized rollups via change-stream worker; Express endpoints for readings, summaries, and meter metadata. Add Joi schemas and rate limits.' },
  { w: 'Week 3', h: 'React dashboard scaffold',
    b: 'Vite + React + Tailwind shell with this layout. Live flow strip via Socket.IO. Daily / weekly bar charts wired to the API. Auth (JWT) and a simple meter switcher.' },
  { w: 'Week 4', h: 'Inference service',
    b: 'Containerize the BiLSTM-Attn × PatchTST ensemble behind FastAPI. Hourly cron pulls last 168h, runs prediction, writes 24h horizon back to Mongo. Add 5-fold OOF for quiet-hour residuals.' },
  { w: 'Week 5', h: 'Forecast view + anomaly engine',
    b: 'Actual-vs-predicted chart with confidence band, residuals strip, and per-rule anomaly detector. Push notifications via web-push for severity ≥ warn.' },
  { w: 'Week 6', h: 'Hardening + deploy',
    b: 'Load test 1k meters at 5s cadence, add PM2 + healthchecks, ship to a small VPS. Optional: Grafana for ops, mobile PWA wrapping for end-users.' }];
  ...
  return (
    <div className="doc">
      ...
      <ul>
        ...
        <li><b style={{ color: 'var(--ink)' }}>Frontend</b> — React 18, Vite, Tailwind, custom SVG charts (no recharts dependency)</li>
...
Object.assign(window, { Rail, Topbar, Overview, Forecast, Anomalies, Architecture, Plan });